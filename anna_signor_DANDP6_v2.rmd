...
A trend become very clear here. Higher quality wines are less and less likely to have a high amount of volatile acidity, and none of the wines graded good or excellent have more than 1.1 volatile acidity. The maroon trend line above is a regression using the LOESS method. I used this throughout the project, to avoid a "liner blinders" effect. If the relationship is not best modeled by a line, I want to be honest about that. In this case, however, what is really interesting is that the LOESS is giving us something extremely similar to a line. The yellow regresssion line is a linear regression, note how much overlap there is. It is only where the distribution becomes sparse that the relationship becomes less linear
